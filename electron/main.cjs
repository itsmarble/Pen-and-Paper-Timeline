  ...
  try {
    ...
    if (!existsSync(optimizedEventsFile)) {
      ...
      if (existsSync(defaultOptimizedPath)) {
        ...
        console.log('Initialized events-optimized.json from default');
      } else {
        ...
        console.log('Created empty events-optimized.json');
      }
    }
...
ipcMain.handle('read-optimized-events', async () => {
  try {
    ...
    console.error('Error reading optimized events:', error);
    return { events: [] };
  }
});
...
ipcMain.handle('write-optimized-events', async (event, optimizedEvents) => {
  try {
    ...
    console.error('Error writing optimized events:', error);
    throw error;
  }
});
...
app.on('before-quit', async () => {
  ...
  try {
    ...
    console.error('Error creating backup on quit:', error);
  }
});

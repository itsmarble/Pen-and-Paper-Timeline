...
export class OptimizedEvent {
  ...
  isActiveAt(gameTime: number): boolean {
    const start = this.getStartDateTime();
  ...
  getSearchScore(searchTerm: string, selectedTags: string[] = []): { score: number; matches: any[] } {
    if (!searchTerm && selectedTags.length === 0) return { score: 1, matches: [] };
    let totalScore = 0;
    let matches: any[] = [];
    const maxScore = 1000;
    if (selectedTags.length > 0) {
      const hasAllTags = selectedTags.every(tag => (this.tags as string[]).some(eventTag => eventTag.toLowerCase().includes(tag.toLowerCase())));
  ...
  normalizeText(text: string): string {
    ...
    const germanReplacements: Record<string, string> = { 'ä': 'ae', 'ö': 'oe', 'ü': 'ue', 'ß': 'ss', 'à': 'a', 'á': 'a', 'â': 'a', 'ã': 'a', 'è': 'e', 'é': 'e', 'ê': 'e', 'ë': 'e', 'ì': 'i', 'í': 'i', 'î': 'i', 'ï': 'i', 'ò': 'o', 'ó': 'o', 'ô': 'o', 'õ': 'o', 'ù': 'u', 'ú': 'u', 'û': 'u', 'ñ': 'n', 'ç': 'c' };
  ...
  tokenize(text: string): string[] {
    ...
    words.forEach((word: string) => { if (word.length > 3) { for (let i = 3; i <= Math.min(word.length, 8); i++) tokens.add(word.substring(0, i)); } });
    words.forEach((word: string) => { if (word.length > 4) { for (let i = Math.max(3, word.length - 5); i < word.length; i++) tokens.add(word.substring(i)); } });
    return Array.from(tokens);
  }

  approximateGermanStem(word: string): string {
    if (!word) return '';
  ...
  calculateFieldScore(fieldText: string, queryTokens: string[], originalQuery: string, baseWeight: number): { score: number; matches: any[] } {
  ...
  getLevenshteinSimilarity(str1: string, str2: string): number {
  ...
  calculateNgramSimilarity(str1: string, str2: string, n: number = 3): number {
  ...
  getNgrams(str: string, n: number): string[] {
    const ngrams = [];
    for (let i = 0; i <= str.length - n; i++) ngrams.push(str.substring(i, i + n));
    return ngrams;
  }

  improvedSoundex(str: string): string {
    if (!str) return '';
    const word = str.toUpperCase().replace(/[^A-Z]/g, '');
    if (!word) return '';
    const firstLetter = word[0];
    const mapping: Record<string, string> = { 'B': '1', 'F': '1', 'P': '1', 'V': '1', 'W': '1', 'C': '2', 'G': '2', 'J': '2', 'K': '2', 'Q': '2', 'S': '2', 'X': '2', 'Z': '2', 'D': '3', 'T': '3', 'L': '4', 'M': '5', 'N': '5', 'R': '6' };
    let code = firstLetter;
    let prevCode = mapping[firstLetter as keyof typeof mapping] || '';
    for (let i = 1; i < word.length && code.length < 6; i++) {
      const currentCode = mapping[word[i] as keyof typeof mapping] || '';
  ...
  improvedSoundexMatch(query: string, text: string): any[] {
    const matches: any[] = [];
...
export class EventCollection {
  ...
  hasChangedSince(lastHash: number): boolean {
  ...
  getSortedByDate(): OptimizedEvent[] {
  ...
  getAllTags(): string[] {
    if (!this._tagCache) {
      ...
      this.events.forEach(event => { (event.tags as string[]).forEach(tag => tagSet.add(tag)); });
      this._tagCache = Array.from(tagSet).sort((a, b) => a.localeCompare(b));
    }
    return this._tagCache;
  }

  search(searchTerm: string = '', selectedTags: string[] = [], options: { minScore?: number; maxResults?: number; sortBy?: string; includeScoring?: boolean; boostRecent?: boolean } = {}): any[] {
    ...
    if (sortBy === 'relevance') {
      ...
      scoredEvents.sort((a, b) => b.event.getStartDateTime().getTime() - a.event.getStartDateTime().getTime());
    }
    // Limit results
  ...
  getSearchSuggestions(searchTerm: string, maxResults: number = 8): string[] {
    ...
    const suggestionArr = Array.from(suggestions) as string[];
    suggestionArr.sort((a, b) => a.localeCompare(b));
    return suggestionArr.slice(0, maxResults);
  }

  preprocessSearchTerm(searchTerm: string): string {
  ...
  getDateRange(): { earliest: Date; latest: Date } | null {
    ...
    const dates = this.events.map(event => event.getStartDateTime()).filter((d): d is Date => !!d);
...
export class EventValidator {
  static validateEvent(eventData: EventData): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    // Name
...
export class EventFormatter {
  ...
  static formatDateRange(startDate: string, startTime: string, endDate?: string, endTime?: string): string {
